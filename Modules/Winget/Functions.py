...
def generate_Installer_Manifest(package_id: str, version: str = None, channel: str = None, auth_token: str = "", serializer = None, use_serializer: bool = True) -> dict:
    with SQLiteDatabase() as db:
    ...
    if package:
        for v_info in versions:
            for inst in v_info.get("Installers", []):
                ...
                if use_serializer and serializer:
                    file_url = serializer.dumps(file_url)
                inst["InstallerUrl"] = url_for("winget_routes.download", package_name=file_url, _external=True)

            for icon in v_info.get("DefaultLocale", {}).get("Icons", []):
                logo_url = icon["IconUrl"]
                if use_serializer and serializer:
                    logo_url = serializer.dumps(logo_url)
                icon["IconUrl"] = url_for("winget_routes.get_package_logo", logo_name=logo_url, _external=True)
...
def _build_installer_entry(installer_data: dict, db: SQLiteDatabase) -> dict:
    installer = {
        ...
        "InstallerUrl": installer_data.get('INSTALLER_URL', ''),
